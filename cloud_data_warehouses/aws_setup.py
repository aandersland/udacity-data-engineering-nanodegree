...
def get_aws_cluster_properties(_redshift, _cluster_identifier):
    ...
    if check_aws_cluster_status(_redshift, _cluster_identifier):
        ...
        return my_cluster_props, dwh_endpoint, dwh_role_arn
    else:
...
def create_cluster():
    ...
    _iam_role_arn = create_aws_iam_role(
        _iam, dwh_iam_role_name,
        "Allows Redshift clusters to call AWS services on your behalf.",
        policy_arn)

    create_redshift_cluster(_redshift, dwh_cluster_type, dwh_node_type,
                            dwh_num_nodes, dwh_cluster_identifier, dwh_db,
                            dwh_db_user, dwh_db_password, _iam_role_arn)

    _cluster_props, dwh_endpoint, dwh_role_arn = get_aws_cluster_properties(
        _redshift, dwh_cluster_identifier)
    ...
    if _cluster_props is None or dwh_endpoint is None or dwh_role_arn is None:
    ...
    open_aws_tcp_port_to_cluster(_ec2, _cluster_props, dwh_port)
